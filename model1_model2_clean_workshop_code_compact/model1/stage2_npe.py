...
from model1 import npe_utils
from model1 import runtime as score_runtime
...
def run_pilot_sanity_checks(
    ...
    contexts = score_contexts(
        runtimes, y_check, first_u, int(args.context_score_batch_size)
    )
    ...
    for method, context in contexts.items():
        ...
        if not np.isfinite(mean_abs) or mean_abs >= float(args.max_mean_abs_score):
            raise AssertionError(
                f"{method} score failed finite/explosion check: mean_abs={mean_abs:.4g}"
            )
        rows.append(
            {
                "method": method,
...
def run(args: argparse.Namespace) -> None:
    ...
    for method in score_methods:
        runtimes[method] = score_runtime.AmortizedScoreRuntime(
            stage1_run_dir, method, device=device
        )
    ...
    if args.training_context_cache is not None:
        ...
        if pilot_cache_path.exists():
            with np.load(pilot_cache_path) as cache:
                ...
                if missing:
                    raise ValueError(
                        f"pilot cache is missing required arrays {missing}: {pilot_cache_path}"
                    )
                ...
                if cache_pi.shape != pi_train.shape or not np.allclose(
                    ...
                    raise ValueError(
                        f"pilot cache pi_train does not match this Stage-2 training set: "
                        f"{pilot_cache_path}"
                    )
                ...
                if "tau" in cache and not np.isclose(
                    float(cache["tau"]), float(metadata["tau"])
                ):
                ...
                pilot_u_train = np.asarray(cache["pilot_u"], dtype=np.float64)
                pilot_status_train = np.asarray(cache["pilot_status"], dtype="U32")
                if pilot_u_train.shape != pi_train.shape:
                    raise ValueError("pilot cache pilot_u shape does not match pi_train")
                if pilot_status_train.shape != pi_train.shape:
                    raise ValueError("pilot cache pilot_status shape does not match pi_train")
                if not np.all(np.isfinite(pilot_u_train)):
                    raise ValueError("pilot cache contains non-finite pilot_u values")
        ...
        contexts = score_contexts(
            ...
            y_train,
            pilot_u_train,
            int(args.context_score_batch_size),
        )
        if "pilot" in methods:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=int,
        default=20260711,
        help="Used only when --training-context-cache supplies a larger cached bank.",
    )
