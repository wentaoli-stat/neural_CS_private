#!/usr/bin/env python3
"""Evaluate frozen Model-1 Stage-1 checkpoints against the exact score.

This command is deliberately separate from :mod:`model1.stage1`: training and
...
def run(args: argparse.Namespace) -> None:
    ...
    if output_csv.exists() or protocol_path.exists():
    ...
    for pi in pi_values:
        ...
        dataset_fingerprints[format(pi, ".17g")] = {
            "y_sha256": array_sha256(eval_data["y"]),
            "exact_truth_sha256": array_sha256(truth),
        }
        for method, score_runtime in runtimes.items():
    ...
    protocol = {
        ...
        "checkpoints": {
            method: {
                "path": f"model_{method}.pt",
                "sha256": checkpoint_sha256(run_dir / f"model_{method}.pt"),
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--pi-values", default="0.10,0.30,0.50,0.65")
