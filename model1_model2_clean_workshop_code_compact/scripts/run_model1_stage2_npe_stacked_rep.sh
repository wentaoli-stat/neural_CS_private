#!/usr/bin/env bash
# Stage-2 replicate of scripts/run_model1_stage2_npe_stacked.sh. REP >= 1 shifts
# every Stage-2 random stream while keeping the protocol identical:
#   NPE simulation bank  --sbi-train-seed 20260723 + REP
#   NPE init / split     --sbi-seed       54000 + 1000*REP
#   posterior sampling   --posterior-seed 87000 + 1000*REP
#   test bank            --test-seeds     100+10*REP ... 109+10*REP
# REP = 0 reproduces the original launcher. No pilot cache is shared, because
# the cache is tied to the NPE simulation bank.
set -euo pipefail

if [[ $# -ne 2 ]]; then
  echo "usage: REP=1 $0 STAGE1_DIR OUTPUT_DIR" >&2
  exit 2
fi

stage1_dir="$1"
output_dir="$2"
rep="${REP:?REP is required}"
for required in \
...
test_seeds="$(seq -s, $((100 + 10 * rep)) $((109 + 10 * rep)))"
...
"${python_bin}" -u -m model1.stage2_npe \
  ...
  --n-sbi-train 50000 --sbi-train-seed $((20260723 + rep)) \
  ...
  --sbi-seed $((54000 + 1000 * rep)) --shared-npe-seed 1 \
  --test-pi-values 0.07,0.10,0.30,0.50,0.65,0.68 \
  --test-seeds "${test_seeds}" \
  --posterior-n 5000 --posterior-seed $((87000 + 1000 * rep)) --shared-posterior-seed 1 \
