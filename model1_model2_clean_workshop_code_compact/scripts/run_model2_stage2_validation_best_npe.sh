...
for required in \
  ...
  "${stage1_dir}/model_linear.pt" "${stage1_dir}/model_shared_radial.pt"; do
...
cd "${repo_root}"
"${python_bin}" -u -m model2.stage2_npe \
  --stage1-run-dir "${stage1_dir}" \
  --methods pilot,linear,shared_radial \
  ...
  --grid-size 5000 \
  --pilot-grid-size 201 --pilot-batch-size 512 --context-score-batch-size 256 \
