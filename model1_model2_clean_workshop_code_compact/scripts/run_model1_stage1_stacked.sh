...
"${python_bin}" -u -m model1.stage1 \
  ...
  --methods linear,radial,stacked \
  --hidden 64 --depth 2 --gate-hidden 16 \
  --m-dim "${M_DIM:-1}" --include-constant-channel "${INCLUDE_CONSTANT_CHANNEL:-1}" \
  ...
  --checkpoint-selection raw \
  --patience 0 --print-every 100 --device "${DEVICE:-auto}" \
