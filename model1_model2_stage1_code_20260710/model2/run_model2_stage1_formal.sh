...
for seed in "${SEEDS[@]}"; do
  "${PYTHON_BIN}" model2/run_blockwise_common_factor_amortized_fsm_experiment.py \
    --n-train 40000 \
    --n-val 8000 \
    --n-test 5000 \
    --n-blocks 40 \
    ...
    --diagnostic-pi-values 0.07,0.10,0.30,0.50,0.65,0.68 \
    --device cuda \
    --output-dir "runs/formal40k_fixed_sigma_seed${seed}"
done

"${PYTHON_BIN}" summarize_amortized_stage1_results.py \
  --runs-dir runs \
  --prefix formal40k_fixed_sigma_seed \
  --output-prefix runs/formal40k_fixed_sigma_summary
