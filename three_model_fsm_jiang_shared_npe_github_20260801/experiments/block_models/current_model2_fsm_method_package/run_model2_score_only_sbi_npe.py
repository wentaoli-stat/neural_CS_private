#!/usr/bin/env python3
"""Score-only NPE using a frozen amortized Model 2 FSM score field.

For every Stage-2 training simulation,

    pi_b ~ prior,
    Y_b ~ p(. | pi_b),
    x_b = S_frozen(Y_b, logit(pi_b)).

The NPE context is exactly the scalar ``x_b``.  The parameter used to evaluate
the score is not concatenated to the context.  Simulated test data use the same
true-parameter score evaluation, making this an oracle-anchor diagnostic of how
much parameter information remains in the score value alone.
"""
...
METHOD_LABELS = {
    "linear": "linear FSM score-only NPE",
    "radial": "radial FSM score-only NPE",
}
...
def score_contexts(
    ...
    for method, runtime in runtimes.items():
        values = runtime.score(y, u_eval, batch_size=int(batch_size)).reshape(-1, 1)
        if values.shape != (y.shape[0], 1):
...
def run(args: argparse.Namespace) -> None:
    ...
    print("\n==== Score-only NPE posterior summary ====")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("runs/model2_score_only_sbi_npe"),
    )
    return parser
