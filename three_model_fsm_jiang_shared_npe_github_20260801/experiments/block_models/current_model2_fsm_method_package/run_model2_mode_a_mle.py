...
def composite_pilot_score_grid(
    ...
    """Equal-channel marginal+pairwise composite estimating function.

    Each channel is averaged over its within-block pieces before summing over
    blocks, preventing the 190 pairwise terms from overwhelming 20 marginal
    terms solely by count.
    """
    lr1 = stage1.marginal_log_ratio(y, float(tau))
    lr2 = stage1.pairwise_log_ratio(y, float(tau))
    ...
    for j, u in enumerate(np.asarray(u_grid, dtype=np.float64)):
        ...
        out[:, j] = s1.mean(axis=2).sum(axis=1) + s2.mean(axis=2).sum(axis=1)
    return out
...
def safeguarded_quasi_newton(
    ...
    *,
    ...
    fallback_lr: float,
    max_backtracks: int,
    initial_secant_step: float,
    batch_size: int,
) -> dict[str, object]:
    """One-dimensional quasi-Newton root solver using secant curvature.

    Unlike ``safeguarded_newton``, this routine never differentiates the score
    network. In one dimension the secant update is the quasi-Newton/BFGS
    curvature update. Safeguards keep every query inside the trained anchor
    support and fall back to score ascent when the secant curvature is invalid.
    """
    u = np.clip(np.asarray(initial_u, dtype=np.float64), runtime.u_min, runtime.u_max)
    n = y.shape[0]
    probe_step = float(initial_secant_step)
    if probe_step <= 0.0:
        raise ValueError("initial_secant_step must be positive")

    probe_direction = np.where(u + probe_step <= runtime.u_max, 1.0, -1.0)
    previous_u = np.clip(u + probe_direction * probe_step, runtime.u_min, runtime.u_max)
    previous_score = runtime.score(y, previous_u, batch_size=batch_size)
    done = np.zeros(n, dtype=bool)
    iterations = np.zeros(n, dtype=np.int64)
    statuses = np.full(n, "max_iter", dtype=object)

    for iteration in range(1, int(max_iters) + 1):
        score = runtime.score(y, u, batch_size=batch_size)
        denominator = u - previous_u
        secant_curvature = np.divide(
            score - previous_score,
            denominator,
            out=np.full(n, np.nan, dtype=np.float64),
            where=np.abs(denominator) > float(step_tol),
        )
        valid_quasi_newton = np.isfinite(secant_curvature) & (
            secant_curvature < -float(min_curvature)
        )
        ...
        statuses[newly_done & left_solution] = "converged_left_boundary"
        statuses[newly_done & right_solution] = "converged_right_boundary"
        statuses[newly_done & interior_solution] = "converged_interior"
        iterations[newly_done] = iteration - 1
        done |= newly_done
        if np.all(done):
            break
        ...
        delta = np.divide(
            -score,
            secant_curvature,
            out=float(fallback_lr) * score,
            where=valid_quasi_newton,
        )
        delta = np.clip(delta, -float(max_step), float(max_step))
        delta[done] = 0.0
        candidate = np.clip(u + delta, runtime.u_min, runtime.u_max)
        ...
        previous_u, previous_score = u.copy(), score.copy()
        u = candidate
        iterations[~done] = iteration

    final_score = runtime.score(y, u, batch_size=batch_size)
    denominator = u - previous_u
    final_curvature = np.divide(
    ...
    right = (u >= runtime.u_max - step_tol) & (final_score >= 0.0)
    interior = (
        (np.abs(final_score) <= score_tol)
        & np.isfinite(final_curvature)
        & (final_curvature < -float(min_curvature))
    )
    statuses[left] = "converged_left_boundary"
    statuses[right] = "converged_right_boundary"
    statuses[interior & ~left & ~right] = "converged_interior"
...
def write_report(
    ...
    lines = [
        ...
        "- The composite pilot uses equal marginal/pairwise channel averages and no exact recovery map.",
        "",
        "## Overall Results",
        "",
        ...
        "- `quasi_newton_secant` uses score values only; its one-dimensional secant update is the BFGS/quasi-Newton analogue.",
...
def run(args: argparse.Namespace) -> None:
    ...
    for pi_index, pi_true in enumerate(pi_true_values):
        ...
        for j, u_value in enumerate(u_grid):
            smooth_score[:, j] = precheck.smoothed_fsm_score_from_full_lr(
                full_lr=full_lr,
                u0=float(u_value),
        ...
        for runtime in runtimes:
            ...
            for initialization, initial_u in (
                ...
                quasi_newton = safeguarded_quasi_newton(
                    runtime,
                    y,
                    initial_u,
                    max_iters=int(args.newton_max_iters),
                    ...
                    fallback_lr=float(args.newton_fallback_lr),
                    max_backtracks=int(args.newton_max_backtracks),
                    initial_secant_step=float(args.quasi_initial_secant_step),
                    batch_size=int(args.batch_size),
                )
                all_rows.extend(
                    estimate_rows(
                        ...
                        pilot_status=pilot_status,
                        method=runtime.method,
                        solver="quasi_newton_secant",
                        initialization=initialization,
                        checkpoint=checkpoint,
                        training_seed=training_seed,
                        estimate_u=np.asarray(quasi_newton["u"]),
                        estimate_status=list(quasi_newton["status"]),
                        converged=np.asarray(quasi_newton["converged"]),
                        iterations=np.asarray(quasi_newton["iterations"]),
                        residual=np.asarray(quasi_newton["score"]),
                    )
                )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--quasi-initial-secant-step", type=float, default=0.05)
    parser.add_argument("--seed", type=int, default=20260709)
    parser.add_argument("--device", default="auto", choices=("auto", "cpu", "cuda"))
    parser.add_argument("--output-dir", default="runs/model2_mode_a_mle_smoke")
    return parser
