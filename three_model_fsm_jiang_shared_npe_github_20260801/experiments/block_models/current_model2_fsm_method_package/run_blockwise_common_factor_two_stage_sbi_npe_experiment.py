#!/usr/bin/env python3
"""Two-stage sbi-NPE for the blockwise common-factor variance mixture.

Model 2:
    Z_k iid Bernoulli(pi), k=1,...,K
    Y_k | Z_k=0 ~ N(0, I_m)
    Y_k | Z_k=1 ~ N(0, I_m + tau^2 11')

Stage 1 trains likelihood-free FSM summaries at a fixed center pi0.
...
METHOD_ORDER = [
    ...
    "positive-MLP radial-gate marginal-only CS DeepSets FSM score",
    "polynomial radial-gate marginal-only CS DeepSets FSM score",
    "linear marginal+pairwise block CS",
    ...
    "positive-MLP radial-gate marginal+pairwise CS linear FSM score",
    "polynomial radial-gate marginal+pairwise CS linear FSM score",
    "MLP radial-gate marginal+pairwise CS DeepSets FSM score",
    "positive-MLP radial-gate marginal+pairwise CS DeepSets FSM score",
    "polynomial radial-gate marginal+pairwise CS DeepSets FSM score",
    "free phi+rho marginal+pairwise CS DeepSets FSM score",
]
...
def simulate_from_pi(
    ...
    return fsm.simulate_from_u(rng, u, int(n_blocks), int(block_size), float(tau))
...
def make_stage1_arrays(y: np.ndarray, pi0: float, tau: float, s1_mean: float, s1_sd: float, s2_mean: float, s2_sd: float):
    s1 = fsm.score_u_from_log_ratio(fsm.marginal_log_ratio(y, tau), pi0)
    s2 = fsm.score_u_from_log_ratio(fsm.pairwise_log_ratio(y, tau), pi0)
    s1_std = ((s1 - s1_mean) / s1_sd).astype(np.float32)
    s2_std = ((s2 - s2_mean) / s2_sd).astype(np.float32)
    struct = np.concatenate([s1_std, s2_std], axis=2)
    block_marg = s1_std.sum(axis=2)
    block_both = np.concatenate([s1_std.sum(axis=2), s2_std.sum(axis=2)], axis=1)
    return struct, block_marg, block_both
...
def train_stage1(args: argparse.Namespace, device: str) -> dict[str, object]:
    ...
    u0 = fsm.logit(pi0)
    print("\n==== Stage 1: common-factor FSM summaries ====")
    ...
    y_train, target_train = fsm.make_fsm_data(
        rng, int(args.stage1_n_train), u0, float(args.sigma_q), int(args.n_blocks), int(args.block_size), float(args.tau)
    )
    y_val, target_val = fsm.make_fsm_data(
        rng, int(args.stage1_n_val), u0, float(args.sigma_q), int(args.n_blocks), int(args.block_size), float(args.tau)
    )

    s1_train = fsm.score_u_from_log_ratio(fsm.marginal_log_ratio(y_train, args.tau), pi0)
    s2_train = fsm.score_u_from_log_ratio(fsm.pairwise_log_ratio(y_train, args.tau), pi0)
    s1_mean, s1_sd = standardize_scalar_fit(s1_train)
    s2_mean, s2_sd = standardize_scalar_fit(s2_train)
    struct_train, block_marg_train, block_train = make_stage1_arrays(
        y_train, pi0, float(args.tau), s1_mean, s1_sd, s2_mean, s2_sd
    )
    struct_val, block_marg_val, block_val = make_stage1_arrays(
        y_val, pi0, float(args.tau), s1_mean, s1_sd, s2_mean, s2_sd
    )
    ...
    block_marg_coef = fsm.fit_linear_ridge(block_marg_train, target_train, ridge=float(args.ridge))
    block_coef = fsm.fit_linear_ridge(block_train, target_train, ridge=float(args.ridge))

    models: dict[str, torch.nn.Module] = {}
    traces: list[dict[str, object]] = []

    if "marginal_linear_deepset" in args.stage1_methods:
        model, trace = fsm.train_torch_model(
            fsm.LinearCSMarginalOnlyDeepSets(int(args.block_size), int(args.hidden)),
            struct_train,
            target_train,
            struct_val,
            target_val,
        ...
        models["marginal_linear_deepset"] = model
        traces.extend({"method": "linear marginal CS DeepSets FSM score", **row} for row in trace)

    if "linear_deepset" in args.stage1_methods:
        model, trace = fsm.train_torch_model(
            fsm.LinearCSMarginalPairwiseDeepSets(int(args.block_size), int(args.hidden)),
            struct_train,
            target_train,
            struct_val,
            target_val,
    ...
    if "marginal_radial_deepset" in args.stage1_methods:
        model, trace = fsm.train_torch_model(
            fsm.RadialGateMarginalOnlyDeepSets(
                int(args.block_size),
                int(args.hidden),
        ...
        models["marginal_radial_deepset"] = model
        traces.extend({"method": f"{gate_label} radial-gate marginal-only CS DeepSets FSM score", **row} for row in trace)

    if "radial_linear" in args.stage1_methods:
        model, trace = fsm.train_torch_model(
            fsm.RadialGateMarginalPairwiseCS(
                int(args.n_blocks),
                int(args.block_size),
                int(args.gate_hidden),
                block_coef,
        ...
        traces.extend({"method": f"{gate_label} radial-gate marginal+pairwise CS linear FSM score", **row} for row in trace)

    if "radial_deepset" in args.stage1_methods:
        model, trace = fsm.train_torch_model(
            fsm.RadialGateMarginalPairwiseDeepSets(
                int(args.block_size),
                int(args.hidden),
                int(args.gate_hidden),
                str(args.clean_gate_kind),
        ...
        models["radial_deepset"] = model
        traces.extend({"method": f"{gate_label} radial-gate marginal+pairwise CS DeepSets FSM score", **row} for row in trace)

    if "free_phi_deepset" in args.stage1_methods:
        model, trace = fsm.train_torch_model(
            fsm.StructuredMarginalPairwiseDeepSets(int(args.block_size), int(args.hidden)),
            struct_train,
            target_train,
            struct_val,
            target_val,
            ...
            print_every=int(args.stage1_print_every),
            device=device,
            name="free phi+rho marginal+pairwise CS DeepSets FSM",
        )
        models["free_phi_deepset"] = model
        traces.extend({"method": "free phi+rho marginal+pairwise CS DeepSets FSM score", **row} for row in trace)
    ...
    return {
        ...
        "s2_sd": s2_sd,
        "raw_coef": raw_coef,
        "block_marg_coef": block_marg_coef,
...
def compute_features(stage1: dict[str, object], y: np.ndarray, args: argparse.Namespace) -> dict[str, np.ndarray]:
    ...
    struct, block_marg, block = make_stage1_arrays(
        y,
        pi0,
        float(args.tau),
    ...
    features: dict[str, np.ndarray] = {
        ...
        "linear marginal block CS": block_marg.astype(np.float32),
        "linear marginal CS ridge FSM score": fsm.predict_linear(
            np.asarray(stage1["block_marg_coef"], dtype=np.float64), block_marg
        ).reshape(-1, 1).astype(np.float32),
        "linear marginal+pairwise block CS": block.astype(np.float32),
        "linear marginal+pairwise CS ridge FSM score": fsm.predict_linear(
    ...
    if "marginal_linear_deepset" in models:
        features["linear marginal CS DeepSets FSM score"] = fsm.predict_torch(
            models["marginal_linear_deepset"], struct, device
        ).reshape(-1, 1).astype(np.float32)
    if "linear_deepset" in models:
        pairwise_linear_ds = fsm.predict_torch(
            models["linear_deepset"], struct, device
        ).reshape(-1, 1).astype(np.float32)
        features["linear CS DeepSets FSM score"] = pairwise_linear_ds
        features["linear marginal+pairwise CS DeepSets FSM score"] = pairwise_linear_ds
    if "marginal_radial_deepset" in models:
        features[f"{gate_label} radial-gate marginal-only CS DeepSets FSM score"] = fsm.predict_torch(
            models["marginal_radial_deepset"], struct, device
        ).reshape(-1, 1).astype(np.float32)
    if "radial_linear" in models:
        features[f"{gate_label} radial-gate marginal+pairwise CS linear FSM score"] = fsm.predict_torch(
            models["radial_linear"], struct, device
        ).reshape(-1, 1).astype(np.float32)
    if "radial_deepset" in models:
        features[f"{gate_label} radial-gate marginal+pairwise CS DeepSets FSM score"] = fsm.predict_torch(
            models["radial_deepset"], struct, device
        ).reshape(-1, 1).astype(np.float32)
    if "free_phi_deepset" in models:
        features["free phi+rho marginal+pairwise CS DeepSets FSM score"] = fsm.predict_torch(
            models["free_phi_deepset"], struct, device
        ).reshape(-1, 1).astype(np.float32)
    return features


def stage1_score_diagnostics(stage1: dict[str, object], args: argparse.Namespace) -> list[dict[str, object]]:
    """Evaluate frozen scalar Stage-1 scores against the exact center score.

    This is diagnostic only; FSM training itself remains likelihood-free.
    """
    if int(args.stage1_diagnostic_n_test) <= 0:
        return []
    rng = np.random.default_rng(int(args.stage1_diagnostic_seed))
    pi0 = float(stage1["pi0"])
    y_eval = fsm.simulate_center(
        rng,
    ...
    exact_score = fsm.score_u_from_log_ratio(fsm.full_log_ratio(y_eval, float(args.tau)), pi0).sum(axis=1)
    ...
    for method, values in features.items():
        ...
        row.update(fsm.metrics(exact_score, values.reshape(-1)))
        rows.append(row)
    return sorted(rows, key=lambda row: float(row["std_mse"]))


def exact_loglik_grid(y: np.ndarray, pi_grid: np.ndarray, tau: float) -> np.ndarray:
    log_r_block = fsm.full_log_ratio(y[None, :, :], float(tau))[0]
    out = []
    for pi in pi_grid:
...
def train_sbi_npe_method(
    ...
    theta = torch.as_tensor(pi_to_unit(pi_train, float(args.pi_prior_min), float(args.pi_prior_max)), dtype=torch.float32)
...
def sample_sbi_posterior(result: dict[str, object], x_obs: np.ndarray, posterior_n: int, seed: int, args: argparse.Namespace) -> np.ndarray:
    ...
    return unit_to_pi(samples_unit.detach().cpu().numpy(), float(args.pi_prior_min), float(args.pi_prior_max))
...
def run(args: argparse.Namespace) -> None:
    ...
    pi_train = sample_pi_prior(rng_train, int(args.n_sbi_train), float(args.pi_prior_min), float(args.pi_prior_max))
    ...
    for idx, method in enumerate(methods):
        results.append(
            train_sbi_npe_method(
                NPE, posterior_nn, BoxUniform, method, feature_dict[method], pi_train,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--sbi-seed", type=int, default=53000)
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("Blockwise Gaussian Mixture/runs/common_factor_two_stage_sbi_npe"))
    return parser
...
def main() -> None:
    ...
    gate_label = fsm.gate_label_for(str(args.clean_gate_kind))
    if gate_label != "MLP":
        args.sbi_methods = str(args.sbi_methods).replace("MLP radial-gate", f"{gate_label} radial-gate")
    run(args)
