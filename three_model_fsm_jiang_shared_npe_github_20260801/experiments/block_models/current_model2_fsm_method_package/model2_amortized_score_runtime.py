...
class AmortizedScoreRuntime:
    ...
    def __init__(self, run_dir: str | Path, method: str, device: str = "auto"):
        ...
        if self.method not in {"linear", "radial"}:
            raise ValueError("method must be 'linear' or 'radial'")
        ...
        if self.config.get("feature_pi_mode") != "anchor":
        ...
        if self.method == "linear":
            ...
            model = stage1.RadialCSBetaDeepSets(
                int(self.config["hidden"]),
                int(self.config["depth"]),
