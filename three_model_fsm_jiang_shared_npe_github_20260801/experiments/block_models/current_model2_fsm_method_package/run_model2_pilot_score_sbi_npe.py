...
from run_model2_mode_a_mle import (
    constrained_modes_from_score_grid,
)
...
CONTEXT_DEFINITION = (
    "(u_pilot(Y), S(Y, u_pilot(Y))); "
    "pilot family selected by --pilot-mode"
)
...
def pilot_score_contexts(
    ...
    pilot_mode: str = "equal_channel",
) -> tuple[dict[str, np.ndarray], np.ndarray, np.ndarray]:
    """Build an oracle-free context while reusing per-chunk log-ratios.

    The torch backend evaluates several pilot-grid locations together on the
    Stage-1 runtime device.  The NumPy backend is retained as a reference path.
    A cached pilot skips the grid sweep but still evaluates the frozen scores
    on the current simulated data.
    """
    if not runtimes:
    ...
    backend = str(pilot_backend).lower()
    if backend == "auto":
        backend = "torch"
    if backend not in {"numpy", "torch"}:
        raise ValueError("pilot_backend must be one of auto, numpy, torch")
    mode = str(pilot_mode).lower()
    if mode not in {"moment", "marginal", "equal_channel"}:
        raise ValueError("pilot_mode must be one of moment, marginal, equal_channel")
    ...
    for chunk_index, start in enumerate(
        ...
        if cached_pilot_u is None:
            if mode == "moment":
                raw_pi = (np.mean(y_chunk**2, axis=(1, 2)) - 1.0) / float(tau) ** 2
                clipped_pi = np.clip(raw_pi, reference.pi_min, reference.pi_max)
                batch_u = np.asarray(stage1.logit_np(clipped_pi), dtype=np.float64)
                batch_status_arr = np.where(
                    raw_pi <= reference.pi_min,
                    "left_boundary",
                    np.where(raw_pi >= reference.pi_max, "right_boundary", "interior"),
                ).astype("U32")
            elif backend == "torch":
                ...
                pilot_field = _numpy_composite_pilot_score_grid(
                    lr1_np,
                    lr2_np,
                    u_grid,
                    include_pairwise=mode == "equal_channel",
        ...
        with torch.inference_mode():
            for method, runtime in runtimes.items():
                score, _ = runtime.score_from_log_ratios_tensor(lr1, lr2, batch_u)
                values = np.column_stack(
                    [batch_u, score.detach().cpu().numpy().astype(np.float64)]
                ).astype(np.float32)
        ...
        if should_report:
            ...
            source = "cache" if cached_pilot_u is not None else f"{mode}/{backend}"
            print(
...
def run_pilot_sanity_checks(
    ...
    for method, values in first.items():
        ...
        if not (float(args.min_mean_abs_score) < mean_abs_score < float(args.max_mean_abs_score)):
            raise AssertionError(
                f"{method} score column failed alive check: mean_abs={mean_abs_score:.4g}"
            )
        rows.append(
            {
                "method": method,
...
def run(args: argparse.Namespace) -> None:
    ...
    if not score_methods:
        raise ValueError("At least one of linear or radial is required to load Stage-1 metadata")
    ...
    if args.training_context_cache is not None:
        ...
        if pilot_cache_path.exists():
            with np.load(pilot_cache_path) as cache:
                ...
                if "pilot_mode" in cache and str(cache["pilot_mode"]) != str(args.pilot_mode):
                    raise ValueError("pilot cache mode does not match --pilot-mode")
        ...
        if cached_pilot_u is None:
            ...
            np.savez_compressed(
                ...
                pilot_mode=np.asarray(str(args.pilot_mode)),
            )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="Data-only pilot family. moment is cheapest; equal_channel is strongest.",
    )
    parser.add_argument(
        "--pilot-backend",
        choices=("auto", "torch", "numpy"),
        default="auto",
        help="Pilot-grid implementation. auto selects the torch device path.",
    )
    parser.add_argument(
