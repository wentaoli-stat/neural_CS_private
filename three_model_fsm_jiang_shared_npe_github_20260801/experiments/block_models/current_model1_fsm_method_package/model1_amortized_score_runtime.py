#!/usr/bin/env python3
"""Load a formal amortized-FSM checkpoint and evaluate its score field.

The runtime deliberately reuses the Stage-1 model classes and frozen feature
statistics.  It exposes the quantity needed by Mode A,

    S(Y, u_eval),  u_eval = logit(pi_eval),

for data generated at any parameter value.  Feature construction remains
differentiable with respect to ``u_eval`` so Newton/Jacobian diagnostics can be
performed without finite-differencing the whole pipeline.
"""
...
class AmortizedScoreRuntime:
    ...
    def __init__(self, run_dir: str | Path, method: str, device: str = "auto"):
        ...
        if self.spec.needs_subscores and self.config.get("feature_pi_mode") != "anchor":
            raise ValueError("Mode A requires a checkpoint trained with feature_pi_mode='anchor'")
    ...
    def score_from_raw_tensor(
        ...
        *,
        ...
        if y_tensor.ndim != 3:
            raise ValueError("raw y tensor must have shape (n, K, m)")
        u_tensor = self._coerce_u(u, int(y_tensor.shape[0]), require_grad=require_grad)
        if self.spec.parameter_input == "anchor_z":
            parameter = ((u_tensor - self.anchor_mean) / self.anchor_sd).to(torch.float32)
        elif self.spec.parameter_input == "physical_u":
            parameter = u_tensor.to(torch.float32)
        else:
            raise ValueError(f"Unknown parameter input mode: {self.spec.parameter_input}")
        return self.model(y_tensor, parameter), u_tensor
        ...
        *,
        ...
        with torch.no_grad():
            for start in range(0, y_arr.shape[0], int(batch_size)):
                ...
                if self.spec.needs_raw_y:
                    score, _ = self.score_from_raw_tensor(y_arr[start:stop], u_arr[start:stop])
                else:
    ...
    def score_grid(
        ...
        *,
        ...
        for start in range(0, y_arr.shape[0], int(batch_size)):
            ...
            lr = None if self.spec.needs_raw_y else self.precompute_log_ratios(y_batch)
            with torch.no_grad():
                for grid_start in range(0, u_values.size, int(grid_chunk_size)):
                    ...
                    if self.spec.needs_raw_y:
                        y_tensor = torch.as_tensor(y_batch, dtype=torch.float32, device=self.device)
                        data_chunk = (
                            y_tensor[:, None, :, :]
                            .expand(-1, n_u, -1, -1)
                            .reshape(n_data * n_u, y_tensor.shape[1], y_tensor.shape[2])
                        )
                    else:
                    ...
                    if self.spec.needs_raw_y:
                        score, _ = self.score_from_raw_tensor(data_chunk, u_repeated)
                    else:
    ...
    def score_and_derivative(
        ...
        *,
        ...
        for start in range(0, y_arr.shape[0], int(batch_size)):
            stop = min(start + int(batch_size), y_arr.shape[0])
            if self.spec.needs_raw_y:
                score, u_tensor = self.score_from_raw_tensor(
                    y_arr[start:stop], u_arr[start:stop], require_grad=True
                )
            else:
                lr = self.precompute_log_ratios(y_arr[start:stop])
    ...
    def legacy_score(self, y: np.ndarray, u: float | np.ndarray, batch_size: int = 256) -> np.ndarray:
        ...
        if self.spec.needs_subscores:
            ...
            feat.update(stage1.featurize(y_arr, self.tau, pi, self.stats))
        if self.spec.needs_raw_y:
            feat["y"] = y_arr.astype(np.float32)
        feat["anchor_z"] = ((u_arr - self.anchor_mean) / self.anchor_sd).astype(np.float32)
        feat["physical_u"] = u_arr.astype(np.float32)
