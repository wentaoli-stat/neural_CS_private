...
## Page 1 — Goal: approximate the full Fisher score using marginal and pairwise composite scores
...
For example, consider a multivariate $t$ model. The score can be written as
...
$$
S_\psi(\theta,\{X_i\}_{i=1}^n,\{(X_i,X_j)\}_{i,j=1}^n)
$$
...
## Why raw score-variance objectives are problematic
...
* MLP with {x_i}
<!-- * MLP with {x_i} and {(x_i, x_i * x_j)} -->
* Linear with {x_i} as in Khoo et al 2025
* MLP with {x_i} under constraints as in Jiang et al 2026 (for amortization, can it learn features such as x^{\theta})
* Linearly aggregated batched composite score
...
Metrics:
* Score approximation error, given the true score function
...
Vector AR with t innnovation: $X_i \in \mathbb{R}^{20}, cs_2(X_i,X_j,\theta)\in\mathbb{R}^2$ 
